...
def calcFARs(embeddings_data: dict, vkey_len_arr: np.array, err_capacity_arr: np.array):
    ...
    for index_user in range(0, number_of_users - 1):
        ...
        embeddings_data_temp = embeddings_data.copy()
        del embeddings_data_temp[list_users[index_user]]
        vector_of_other_users = np.sign(np.array(list(embeddings_data_temp.values())).reshape(-1, 512))
        
        for test_vector in vector_of_other_users:
            total_imposter_trial+=1
            intra_hamming = calc_hamming_dictance(vector_of_user, test_vector)
